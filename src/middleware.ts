...
export async function middleware(request: NextRequest) {
    ...
    // Função auxiliar para aplicar cookies no redirecionamento (caso necessário)
    const applyCookies = (redirectRes: NextResponse) => {
        const supabaseCookies = supabaseResponse.cookies.getAll()
        console.log(`[Middleware] Aplicando ${supabaseCookies.length} cookies ao redirecionamento`)
        return redirectRes
    }
