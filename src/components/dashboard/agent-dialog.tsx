...
const MODELS_METADATA: Record<string, { label: string; badge?: string; badgeVariant?: "default" | "secondary" | "outline" | "destructive" }> = {
    /** -------- OpenAI -------- */
    'openai:gpt-4o': {
        label: 'GPT-4o',
        badge: 'Flagship',
                                ...
                                {allowedModels.map(m => (
                                    <SelectItem key={m} value={m}>
                                        <div className="flex items-center justify-between gap-2 w-full min-w-[180px]">
                                            <span>{MODELS_METADATA[m]?.label || m}</span>
                                            {MODELS_METADATA[m]?.badge && (
                                                <Badge variant={MODELS_METADATA[m].badgeVariant} className="text-[10px] uppercase px-1.5 py-0">
                                                    {MODELS_METADATA[m].badge}
                                                </Badge>
                                            )}
                                        </div>
