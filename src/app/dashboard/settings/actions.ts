...
export async function updateSettings(prevState: any, formData: FormData): Promise<SettingsState> {
    ...
    const processKey = (formKey: string, dbColumn: string, prefix: string) => {
        ...
        if (!trimmedKey.startsWith(prefix)) {
            throw new Error(`A chave informada está com formato inválido (deve começar com '${prefix}').`)
        }
    ...
    if (action === 'delete_openai') {
    ...
    } else if (action === 'delete_deepseek') {
        updates['api_key_deepseek'] = null as any;
    } else {
        ...
        try {
            ...
            processKey('apiKeyDeepSeek', 'api_key_deepseek', 'sk-')
        } catch (err: any) {
